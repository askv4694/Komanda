...
set.seed(2)
sample<-data[sample(nrow(data[1:2000,])),]

```
Padaryta krosvalidacija (10 folds), išskirtos 2 grupės : testuojama, t.y kuri bus nežinomoji ir žinomoji, su kuria lyginsime. Gauti vidurkiai, tačiau jie nėra tobuli, iš 10 bandymų pirmas ir septintas turi labai dideles paklaidas.
```{r}
...
print("Paskutinės iteracijos, klaidos tikimybės eilutė.(Kiekvienam nežinomam mėginiui)")
frez
print("Paklaidos vidurkis")
mean(frez)
print("Tikslumo vidurkis")
mean(srez)
print("Confusion matrix, kur TrueA - teisingai nustatytas Alzheimeris, FalseA - neteisingai, C -control.")
conf
```

Čia bandymas su 8 chromosoma, tačiau taip pat pirmas ir septintas yra klaidingi.
```{r}
...
for(i in 1:10){
  ...
  knownData <- as.data.frame(knownData)
  #Jei paskutinis = pval, tada naudojamas t.test sumažinti duomenims
  k <- c(3, 5, 7,8, 9, 10, 11, 12, 13, 15)
  group <- getGroup(testData, knownData, k[i], "")
  tested <- getTested(testData)
  mat <- as.data.frame(cbind(group, tested))
  print(paste0("Tikslumas nuo k = " ,k[i]))
  print(confMatrix(mat, conf))
  frez[i] <- getRate(mat, "f")
  srez[i] <- getRate(mat)
}
print("Paklaidos vidurkis")
